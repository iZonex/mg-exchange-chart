...
// Like Schiff Pitchfork but handle = midpoint(p1, p2) instead of
// midpoint(p1, midpoint(p2, p3)). Creates a different median slope.
...
export const modifiedSchiffPitchforkTool: InternalDrawingTool = {
  name: 'modified-schiff-pitchfork',
  icon: 'M12,3 L12,21 M5,11 L12,3 L19,11',
  pointCount: 3,
  ...
  render(ctx, points, options, width, height) {
    ...
    // Modified Schiff: handle = midpoint(p1, p2)
    const handle = { x: (p1.x + p2.x) / 2, y: (p1.y + p2.y) / 2 };
