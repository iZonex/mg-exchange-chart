// ── Elliott Triple Combination (WXYXZ) ──────────────────────────────────────
// 6 points: W, X, Y, X, Z labels. Three corrective waves connected.
// Point sequence: start → W → X → Y → X2 → Z
...
export const elliottTripleComboTool: InternalDrawingTool = {
  name: 'elliott-triple-combo',
  icon: 'M2,16 L5,6 L8,14 L11,4 L15,12 L20,3',
  pointCount: 6, // start, W, X, Y, X2, Z

  render(ctx, points, options) {
    if (points.length < 2) return;

    const labels = ['\u2022', t('wLabel'), t('xLabel'), t('yLabel'), t('xLabel'), t('zLabel')];
    ...
    // Dashed line from start to Z (overall move)
    if (points.length >= 6) {
      ctx.setLineDash([3, 3]);
      ctx.globalAlpha = 0.3;
      ctx.beginPath();
    ...
    if (points.length >= 6) {
      ...
      if (data) {
        const cx = (points[0]!.x + points[5]!.x) / 2;
        const cy = Math.max(points[0]!.y, points[5]!.y) + 20;
